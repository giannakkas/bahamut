...
function CandidatesSection({ candidates }: { candidates: any[] }) {
  ...
  const urgency = (s: number) => s >= THRESHOLD ? { label: "READY", cls: "bg-green-500/25 text-green-400 border-green-500/40" } :
    ...
    { label: "WEAK", cls: "bg-bah-border/50 text-bah-muted border-bah-border" };
          ...
          <table className="w-full text-[11px] min-w-[1000px]">
            ...
            <tbody>
              {candidates.map((c: any, i: number) => {
                ...
                return (
                  <tr key={i} className={`border-b border-bah-border hover-row anim-slide ${isTop ? "bg-bah-cyan/[0.03]" : isAbove ? "bg-bah-surface" : ""}`} style={{ animationDelay: `${i * 0.035}s` }}>
                    <td className="px-3 py-2.5">
                      <div className="flex items-center gap-2">
                        <div className="w-12 h-2 bg-bah-border/50 rounded-full overflow-hidden">
                        ...
                        <span className={`text-xs font-bold px-2 py-0.5 rounded border min-w-[32px] text-center ${
                          ...
                          "text-bah-muted bg-bah-border/50 border-bah-border"
                        }`}>{c.score}</span>
                      </div>
                    </td>
                    <td className="px-3 py-2.5">
                      ...
                      <div className="text-[9px] text-bah-muted mt-0.5">{c.asset_class}</div>
                    </td>
                    <td className="px-3 py-2.5 text-bah-text font-medium">{c.strategy}</td>
                    ...
                    <td className="px-3 py-2.5"><span className={`px-1.5 py-0.5 rounded text-[9px] font-bold ${
                      c.regime === "TREND" || c.regime === "BREAKOUT" ? "bg-green-500/12 text-green-400 border border-green-500/25" :
                    ...
                    <td className="px-3 py-2.5 text-[10px] text-bah-text font-mono">{c.distance_to_trigger}</td>
                    <td className="px-3 py-2.5 font-mono"><span className={`font-semibold ${c.indicators?.rsi < 30 ? "text-green-400" : c.indicators?.rsi > 70 ? "text-red-400" : "text-bah-text"}`}>{c.indicators?.rsi?.toFixed(0) || "—"}</span></td>
                    ...
                    <td className="px-3 py-2.5 max-w-[260px]">
                      {(c.reasons || []).slice(0, 3).map((r: string, j: number) => (
                        <div key={j} className="text-[10px] text-bah-muted leading-snug">{r}</div>
                      ))}
                    </td>
                  </tr>
                );
