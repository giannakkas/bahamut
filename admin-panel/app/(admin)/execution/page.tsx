                ...
                <div key={asset} className="bg-bah-surface border border-amber-500/20 rounded-xl p-4">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-3">
                      <span className={`text-2xl ${d.direction === "LONG" ? "text-green-400" : "text-red-400"}`}>
                        {d.direction === "LONG" ? "▲" : "▼"}
                      </span>
                      <div>
                        ...
                        <div className="text-xs text-bah-muted">{d.decision} · BALANCED · APPROVAL</div>
                      </div>
                    </div>
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => handleApprove(asset)} disabled={approving === asset}
                          ...
                          {approving === asset ? "..." : "Approve Trade"}
                        </button>
                        <button className="px-3 py-2 bg-red-500/20 text-red-400 border border-red-500/30 rounded-lg text-xs font-semibold hover:bg-red-500/30 transition-colors">
                          Reject
                        </button>
                      </div>
                    </div>
                  ...
                  {agents.length > 0 && (
                    <div className="flex gap-2 flex-wrap">
                      {agents.slice(0, 6).map((a: any, i: number) => (
                        <span key={i} className={`px-2.5 py-1 rounded-full text-[10px] font-semibold border ${
                          a.directional_bias === d.direction ? "bg-green-500/10 text-green-400 border-green-500/30" :
                          "bg-red-500/10 text-red-400 border-red-500/30"
                        }`}>
                          {(a.agent_id || "").replace("_agent", "").replace("_", " ")} ▼{((a.confidence || 0) * 100).toFixed(0)}%
                        </span>
                      ))}
                    </div>
                  )}
              ...
              return (
                <div key={asset} className="flex items-center justify-between px-4 py-2 border-b border-bah-border/30 last:border-0">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs text-bah-muted">{d.direction || "—"} · {d.decision || "NO_TRADE"}</span>
                  </div>
